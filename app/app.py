...
class StatusRecSession(object):
    def on_get(self, req, resp):
        """Handles GET requests"""
        status = utils.status_rec()
        if status[0] == 'true':
            info = utils.info_rec(status[1])
...
app.add_route('/stop_preview', stop_preview)
